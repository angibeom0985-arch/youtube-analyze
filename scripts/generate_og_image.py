...
def create_og_image(output_path, title_text, subtitle_text, bg_color, width, height):
    ...
    title_y = int(height * 0.32)  # 상대적 위치
    
    subtitle_bbox = draw.textbbox((0, 0), subtitle_text, font=subtitle_font)
    subtitle_width = subtitle_bbox[2] - subtitle_bbox[0]
    subtitle_x = (width - subtitle_width) // 2
    subtitle_y = int(height * 0.51)  # 상대적 위치
    ...
    domain_y = int(height * 0.76)  # 상대적 위치
...
print("\n모든 OG 이미지 생성이 완료되었습니다! (직사각형 + 정사각형)")
